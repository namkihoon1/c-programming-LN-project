...
void decreaseWeight(Player* player) {
    ...
    printf("무게가 %dkg 감소했습니다! 현재 무게: %dkg\n", player->weightChange, player->currentWeight); // 일단 무게 증가나 무게 감소 배율?은 이후 조정. 
}
...
void playGame(Player* player) {
    ...
    while (player->currentFloor <= MAX_FLOORS && player->currentWeight > 0) {
        ...
        if (player->previousEffectFloor == 1) {
            player->previousEffectFloor--;
            player->currentWeight = player->currentWeight + player->weightChange;
        }
